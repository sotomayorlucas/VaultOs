...
; Catch-all stub for uninitialized IDT entries (vectors 48-255).
; Pushes dummy error code + vector 0xFF, then goes through the normal handler
; so isr_handler() can send EOI / log and return safely.
[global unhandled_interrupt_stub]
unhandled_interrupt_stub:
    push qword 0            ; Dummy error code
    push qword 0xFF         ; Sentinel vector = 255
    jmp isr_common_stub

; Common ISR stub: save all registers, call C handler, restore, iretq
isr_common_stub:
    ; Check if we came from Ring 3 by examining CS on the interrupt frame.
    ; Stack at this point: [rsp+16] = error_code, [rsp+8] = vector
    ; and below that the CPU-pushed frame: RIP, CS, RFLAGS, RSP, SS
    ; After our two pushes (vector + error_code), CS is at [rsp + 24].
    ; Actually: our pushes are vector and error_code (each 8 bytes = 16 bytes)
    ; CPU frame starts at rsp+16: RIP at [rsp+16], CS at [rsp+24]
...
.no_swapgs_entry:
    ...
    ; Check if returning to Ring 3 — CS is now at [rsp + 16] after removing
    ; 15 GPRs (vector and error_code still on stack, then CPU frame)
    test qword [rsp + 24], 3   ; CS is at [rsp + 8(vector) + 8(errcode) + 8(RIP)] = rsp+24
    jz .no_swapgs_exit
    swapgs
.no_swapgs_exit:
