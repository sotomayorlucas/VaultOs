...
\node[layer=blue!40,above=0pt of cp,
  font=\sffamily\small\color{white}]        (sh)  {VaultShell: SQL Query REPL};
\end{tikzpicture}
\vspace{2cm}
...
\node[block,fill=gray!10]                     (hw)  {x86\_64 Hardware: CPU, RAM, PIC, PIT, PS/2, COM1, GOP};
\node[block,fill=blue!8,above=4pt of hw]      (drv) {Drivers: Serial, Framebuffer, Keyboard, Font};
...
\node[block,fill=blue!40,above=4pt of db,
  text=white]                                  (sh)  {VaultShell: SQL REPL $\mid$ Line Editor $\mid$ Display Formatter};
...
The database engine indexes every table with a B-tree of order $t = 64$,
providing $\BigO(\log_{64} n)$ search, insert, and delete operations.

\section{B-Tree Properties}

\begin{definition}[B-Tree of Order $t$]
A B-tree of order $t$ satisfies:
\begin{enumerate}
\item Every node has at most $2t - 1$ keys ($= 127$ for $t = 64$).
\item Every non-root node has at least $t - 1$ keys ($= 63$).
...
\noindent
In \VaultOS{}, BTREE\_ORDER = 64, so nodes store up to 63 keys and 64 child pointers.
Each node occupies approximately $63 \times 8 + 63 \times 8 + 64 \times 8 + 8 = 1{,}520$
bytes, fitting comfortably in L1 cache.

\begin{remark}
We use order-$t$ notation where the maximum number of keys per node is $t - 1 = 63$,
consistent with the CLRS definition. Some references use $t$ for the maximum keys;
ours uses $t$ for the minimum degree.
\end{remark}
...
A B-tree of order $t = 64$ containing $n$ keys has height
$h \leq \log_{64}\!\bigl(\frac{n+1}{2}\bigr)$.
\end{theorem}
\begin{proof}
The minimum number of keys at depth $d$ is $2 \cdot 63^{d-1}$ for $d \geq 1$ (the root
has at least 1 key, each other node at least 63). Summing gives
$n \geq 1 + 2\sum_{i=1}^{h-1} 63^i = 2 \cdot 64^{h-1} - 1$, yielding
$h \leq 1 + \log_{64}\!\bigl(\frac{n+1}{2}\bigr)$.
\end{proof}

\begin{corollary}
For $n = 10^6$ keys, $h \leq 1 + \log_{64}(500{,}000) \approx 4.15$, so the tree has at
...
\section{Per-Table Key Derivation}

Each table receives a unique AES-128 key derived from the master database key:

\begin{algorithm}[H]
\caption{\textsc{Derive-Table-Key}($\text{table\_id}$): Derive AES key for table encryption.}
\label{alg:derive-key}
$\text{id\_buf}[0..3] \leftarrow$ 32-bit encoding of table\_id \;
$\text{derived}[0..31] \leftarrow \textsc{HMAC-SHA256}(K_{\text{master}}, \text{id\_buf}, 4)$ \;
$\textsc{AES-Init}(\text{table\_ctx}[\text{table\_id}], \text{derived}[0..15])$ \tcp*{first 16 bytes}
\end{algorithm}
...
\centering
\begin{tikzpicture}[
  box/.style={draw,thick,rounded corners=3pt,minimum width=2.2cm,
    minimum height=0.7cm,font=\sffamily\footnotesize,fill=blue!8},
  arr/.style={->,>=Stealth,thick},
]
\node[box]                       (tok) {Tokenize};
\node[box,right=0.5cm of tok]    (par) {Parse};
\node[box,right=0.5cm of par]    (cap) {Cap Check};
\node[box,right=0.5cm of cap]    (exe) {Execute};
\node[box,right=0.5cm of exe]    (enc) {Encrypt};
\node[box,right=0.5cm of enc]    (res) {Result};
\draw[arr] (tok) -- (par);
\draw[arr] (par) -- (cap);
\draw[arr] (cap) -- (exe);
\draw[arr] (exe) -- (enc);
\draw[arr] (enc) -- (res);
\end{tikzpicture}
\caption{Query execution pipeline. Capability checks occur before any data access.}
