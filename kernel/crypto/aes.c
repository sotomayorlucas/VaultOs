#include "aes.h"
#include "../lib/string.h"
#include "../arch/x86_64/cpu.h"

/*
 * AES-128 implementation (based on FIPS 197).
 * Uses precomputed GF(2^8) multiplication tables for speed.
 * AES-NI hardware acceleration when available.
 */

/* Runtime flag: set once at aes_init time */
static bool aesni_available = false;
static bool aesni_checked = false;

/* S-box */
static const uint8_t sbox[256] = {
    
};

/* Inverse S-box */
static const uint8_t inv_sbox[256] = {
    
};

/* Round constants */
static const uint8_t rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

/* Precomputed GF(2^8) multiplication tables */
static const uint8_t mul2[256] = {
    
};

static const uint8_t mul3[256] = {
    
};

static const uint8_t mul9[256] = {
    
};

static const uint8_t mul11[256] = {
    
};

static const uint8_t mul13[256] = {
    
};

static const uint8_t mul14[256] = {
    
};

/* Key expansion */
void aes_init(aes_ctx_t *ctx, const uint8_t key[16]) {
    /* Check AES-NI availability once */
    if (!aesni_checked) {
        uint32_t eax, ebx, ecx, edx;
        cpuid(1, &eax, &ebx, &ecx, &edx);
        aesni_available = (ecx >> 25) & 1;
        aesni_checked = true;
    }

    memcpy(ctx->round_key, key, 16);

    for (int i = 4; i < 44; i++) {
        uint8_t temp[4];
        memcpy(temp, ctx->round_key + (i - 1) * 4, 4);

        if (i % 4 == 0) {
            /* RotWord */
            uint8_t t = temp[0];
            temp[0] = temp[1]; temp[1] = temp[2];
            temp[2] = temp[3]; temp[3] = t;

            /* SubWord */
            temp[0] = sbox[temp[0]]; temp[1] = sbox[temp[1]];
            temp[2] = sbox[temp[2]]; temp[3] = sbox[temp[3]];

            /* XOR with Rcon */
            temp[0] ^= rcon[i / 4];
        }

        for (int j = 0; j < 4; j++) {
            ctx->round_key[i * 4 + j] = ctx->round_key[(i - 4) * 4 + j] ^ temp[j];
        }
    }
}

static void sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = sbox[state[i]];
}

static void inv_sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = inv_sbox[state[i]];
}

static void shift_rows(uint8_t state[16]) {
    uint8_t tmp;
    /* Row 1: shift left 1 */
    tmp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = tmp;
    /* Row 2: shift left 2 */
    tmp = state[2]; state[2] = state[10]; state[10] = tmp;
    tmp = state[6]; state[6] = state[14]; state[14] = tmp;
    /* Row 3: shift left 3 (= shift right 1) */
    tmp = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = tmp;
}

static void inv_shift_rows(uint8_t state[16]) {
    uint8_t tmp;
    /* Row 1: shift right 1 */
    tmp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = tmp;
    /* Row 2: shift right 2 */
    tmp = state[2]; state[2] = state[10]; state[10] = tmp;
    tmp = state[6]; state[6] = state[14]; state[14] = tmp;
    /* Row 3: shift right 3 (= shift left 1) */
    tmp = state[3]; state[3] = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = tmp;
}

/* MixColumns using precomputed lookup tables (replaces gmul loops) */
static void mix_columns(uint8_t state[16]) {
    for (int c = 0; c < 4; c++) {
        int i = c * 4;
        uint8_t a0 = state[i], a1 = state[i+1], a2 = state[i+2], a3 = state[i+3];
        state[i]   = mul2[a0] ^ mul3[a1] ^ a2 ^ a3;
        state[i+1] = a0 ^ mul2[a1] ^ mul3[a2] ^ a3;
        state[i+2] = a0 ^ a1 ^ mul2[a2] ^ mul3[a3];
        state[i+3] = mul3[a0] ^ a1 ^ a2 ^ mul2[a3];
    }
}

static void inv_mix_columns(uint8_t state[16]) {
    for (int c = 0; c < 4; c++) {
        int i = c * 4;
        uint8_t a0 = state[i], a1 = state[i+1], a2 = state[i+2], a3 = state[i+3];
        state[i]   = mul14[a0] ^ mul11[a1] ^ mul13[a2] ^ mul9[a3];
        state[i+1] = mul9[a0] ^ mul14[a1] ^ mul11[a2] ^ mul13[a3];
        state[i+2] = mul13[a0] ^ mul9[a1] ^ mul14[a2] ^ mul11[a3];
        state[i+3] = mul11[a0] ^ mul13[a1] ^ mul9[a2] ^ mul14[a3];
    }
}

static void add_round_key(uint8_t state[16], const uint8_t *rk) {
    for (int i = 0; i < 16; i++) state[i] ^= rk[i];
}

/* Software AES encrypt/decrypt */
static void aes_encrypt_block_sw(aes_ctx_t *ctx, uint8_t block[16]) {
    add_round_key(block, ctx->round_key);
    for (int round = 1; round < AES_NUM_ROUNDS; round++) {
        sub_bytes(block);
        shift_rows(block);
        mix_columns(block);
        add_round_key(block, ctx->round_key + round * 16);
    }
    sub_bytes(block);
    shift_rows(block);
    add_round_key(block, ctx->round_key + AES_NUM_ROUNDS * 16);
}

static void aes_decrypt_block_sw(aes_ctx_t *ctx, uint8_t block[16]) {
    add_round_key(block, ctx->round_key + AES_NUM_ROUNDS * 16);
    for (int round = AES_NUM_ROUNDS - 1; round >= 1; round--) {
        inv_shift_rows(block);
        inv_sub_bytes(block);
        add_round_key(block, ctx->round_key + round * 16);
        inv_mix_columns(block);
    }
    inv_shift_rows(block);
    inv_sub_bytes(block);
    add_round_key(block, ctx->round_key);
}

/* AES-NI hardware-accelerated encrypt/decrypt */
static void aes_encrypt_block_ni(aes_ctx_t *ctx, uint8_t block[16]) {
    __asm__ volatile (
        "movdqu (%0), %%xmm0\n\t"       /* Load block */
        "movdqu (%1), %%xmm1\n\t"       /* Round key 0 */
        "pxor   %%xmm1, %%xmm0\n\t"     /* AddRoundKey */
        "movdqu 16(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDC,0xC1\n\t"  /* aesenc xmm0, xmm1 */
        "movdqu 32(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDC,0xC1\n\t"
        "movdqu 48(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDC,0xC1\n\t"
        "movdqu 64(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDC,0xC1\n\t"
        "movdqu 80(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDC,0xC1\n\t"
        "movdqu 96(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDC,0xC1\n\t"
        "movdqu 112(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDC,0xC1\n\t"
        "movdqu 128(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDC,0xC1\n\t"
        "movdqu 144(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDC,0xC1\n\t"
        "movdqu 160(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDD,0xC1\n\t"  /* aesenclast xmm0, xmm1 */
        "movdqu %%xmm0, (%0)\n\t"       /* Store result */
        :
        : "r"(block), "r"(ctx->round_key)
        : "xmm0", "xmm1", "memory"
    );
}

static void aes_decrypt_block_ni(aes_ctx_t *ctx, uint8_t block[16]) {
    /* aesdec requires InvMixColumns-transformed round keys for rounds 1-9.
       aesimc (0x66 0x0F 0x38 0xDB /r) transforms each key before use. */
    __asm__ volatile (
        "movdqu (%0), %%xmm0\n\t"       /* Load block */
        "movdqu 160(%1), %%xmm1\n\t"    /* Round key 10 (no aesimc) */
        "pxor   %%xmm1, %%xmm0\n\t"
        "movdqu 144(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDB,0xC9\n\t"  /* aesimc xmm1, xmm1 */
        ".byte 0x66,0x0F,0x38,0xDE,0xC1\n\t"  /* aesdec xmm0, xmm1 */
        "movdqu 128(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDB,0xC9\n\t"
        ".byte 0x66,0x0F,0x38,0xDE,0xC1\n\t"
        "movdqu 112(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDB,0xC9\n\t"
        ".byte 0x66,0x0F,0x38,0xDE,0xC1\n\t"
        "movdqu 96(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDB,0xC9\n\t"
        ".byte 0x66,0x0F,0x38,0xDE,0xC1\n\t"
        "movdqu 80(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDB,0xC9\n\t"
        ".byte 0x66,0x0F,0x38,0xDE,0xC1\n\t"
        "movdqu 64(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDB,0xC9\n\t"
        ".byte 0x66,0x0F,0x38,0xDE,0xC1\n\t"
        "movdqu 48(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDB,0xC9\n\t"
        ".byte 0x66,0x0F,0x38,0xDE,0xC1\n\t"
        "movdqu 32(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDB,0xC9\n\t"
        ".byte 0x66,0x0F,0x38,0xDE,0xC1\n\t"
        "movdqu 16(%1), %%xmm1\n\t"
        ".byte 0x66,0x0F,0x38,0xDB,0xC9\n\t"
        ".byte 0x66,0x0F,0x38,0xDE,0xC1\n\t"
        "movdqu (%1), %%xmm1\n\t"        /* Round key 0 (no aesimc) */
        ".byte 0x66,0x0F,0x38,0xDF,0xC1\n\t"  /* aesdeclast xmm0, xmm1 */
        "movdqu %%xmm0, (%0)\n\t"
        :
        : "r"(block), "r"(ctx->round_key)
        : "xmm0", "xmm1", "memory"
    );
}

/* Public API: dispatch to AES-NI or software */
void aes_encrypt_block(aes_ctx_t *ctx, uint8_t block[16]) {
    if (aesni_available) {
        aes_encrypt_block_ni(ctx, block);
    } else {
        aes_encrypt_block_sw(ctx, block);
    }
}

void aes_decrypt_block(aes_ctx_t *ctx, uint8_t block[16]) {
    if (aesni_available) {
        aes_decrypt_block_ni(ctx, block);
    } else {
        aes_decrypt_block_sw(ctx, block);
    }
}

/* Optimized CBC: avoid redundant memcpy by using pointer to previous ciphertext */
void aes_cbc_encrypt(aes_ctx_t *ctx, const uint8_t iv[16],
                      const uint8_t *plaintext, uint8_t *ciphertext,
                      size_t length) {
    const uint8_t *prev = iv;
    for (size_t off = 0; off < length; off += 16) {
        for (int i = 0; i < 16; i++)
            ciphertext[off + i] = plaintext[off + i] ^ prev[i];
        aes_encrypt_block(ctx, ciphertext + off);
        prev = ciphertext + off;
    }
}

void aes_cbc_decrypt(aes_ctx_t *ctx, const uint8_t iv[16],
                      const uint8_t *ciphertext, uint8_t *plaintext,
                      size_t length) {
    const uint8_t *prev = iv;
    for (size_t off = 0; off < length; off += 16) {
        uint8_t block[16];
        memcpy(block, ciphertext + off, 16);
        aes_decrypt_block(ctx, block);
        for (int i = 0; i < 16; i++)
            plaintext[off + i] = block[i] ^ prev[i];
        prev = ciphertext + off;
    }
}

size_t aes_padded_size(size_t data_len) {
    return ((data_len / 16) + 1) * 16;
}

void aes_pkcs7_pad(uint8_t *buf, size_t data_len, size_t buf_len) {
    uint8_t pad_byte = (uint8_t)(buf_len - data_len);
    for (size_t i = data_len; i < buf_len; i++) {
        buf[i] = pad_byte;
    }
}

size_t aes_pkcs7_unpad(const uint8_t *buf, size_t buf_len) {
    if (buf_len == 0 || buf_len % 16 != 0) return 0;
    uint8_t pad = buf[buf_len - 1];
    if (pad == 0 || pad > 16) return 0;
    for (size_t i = buf_len - pad; i < buf_len; i++) {
        if (buf[i] != pad) return 0;
    }
    return buf_len - pad;
}
