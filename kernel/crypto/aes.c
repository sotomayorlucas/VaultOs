 ...
 * Software-only, no side-channel hardening beyond constant-time S-box.
 */
...
void aes_cbc_encrypt(aes_ctx_t *ctx, const uint8_t iv[16],
    ...
    for (size_t off = 0; off < length; off += 16) {
        uint8_t block[16];
        memcpy(block, plaintext + off, 16);
        for (int i = 0; i < 16; i++) block[i] ^= prev[i];
        aes_encrypt_block(ctx, block);
        memcpy(ciphertext + off, block, 16);
